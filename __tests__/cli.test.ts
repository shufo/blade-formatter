...
describe("The blade formatter CLI", () => {
	test.concurrent("should print the help", async () => {
	...
	test.concurrent(
		...
		async () => {
			const response = cmd.executeSync(binPath, [
				path.resolve("__tests__", "fixtures", "index.blade.php"),
				"-c",
			]);
	...
	test.concurrent("should ignore commented lines #8", async () => {
		const formatted = await cmd.execute(binPath, [
	...
	test.concurrent("it can format php tag mixed file", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve("__tests__", "fixtures", "formatted_shorttag.blade.php"),
		);
	...
	test.concurrent(
		...
		async () => {
			...
			const formatted = fs.readFileSync(
				path.resolve("__tests__", "fixtures", "formatted.if_nest.blade.php"),
			);
	...
	test.concurrent("stdin option", async () => {
		const cmdResult = await spawnSync(
			"/bin/cat",
			["__tests__/fixtures/index.blade.php", "|", binPath, "--stdin"],
			{
				stdio: "pipe",
				shell: true,
			},
	...
	test.concurrent("stdin option should respects runtime config", async () => {
		const cmdResult = await spawnSync(
			...
			[
				path.resolve(
					"__tests__/fixtures/runtimeConfig/indentSize/index.blade.php",
				),
				"|",
	...
	test.concurrent("@props directive", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve("__tests__", "fixtures", "formatted.props.blade.php"),
		);
	...
	test.concurrent("can fomrat alpine attributes", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve(
				"__tests__",
				"fixtures",
				"formatted.alpine_attributes.blade.php",
			),
		);
	...
	test.concurrent("specify wrap attributes config by rc file", async () => {
		const cmdResult = await cmd.execute(binPath, [
			...
			path.resolve(
				...
				"fixtures",
				"runtimeConfig",
				".bladeformatterrc.force-aligned",
			),
	...
	test.concurrent(
		"specify sort tailwindcss classes config by rc file",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				"--config",
				path.resolve(
					...
					"fixtures",
					"runtimeConfig",
					".bladeformatterrc.sort-tailwind-classes",
				),
			...
			const formatted = fs.readFileSync(
				path.resolve(
					"__tests__",
					"fixtures",
					"formatted.tailwindcss.blade.php",
				),
			);
	...
	test.concurrent(
		"runtime config test for no multiple empty lines",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve(
					"__tests__",
	...
	test.concurrent("sort html attributes option", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve(
				"__tests__",
				"fixtures",
				"formatted.sort_html_attribute.blade.php",
			),
		);
	...
	test.concurrent("sort html attributes runtime config", async () => {
		const cmdResult = await cmd.execute(binPath, [
			...
			path.resolve(
				...
				"fixtures",
				"runtimeConfig",
				".bladeformatterrc.sort-html-attributes",
			),
			path.resolve("__tests__", "fixtures", "sort_html_attribute.blade.php"),
		]);
		...
		const formatted = fs.readFileSync(
			path.resolve(
				"__tests__",
				"fixtures",
				"formatted.sort_html_attribute.alphabetical.blade.php",
			),
		);
	...
	test.concurrent(
		"specify tailwind.config.js path (absolute path)",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve("__tests__", "fixtures", "tailwind", "index.blade.php"),
				"--sort-tailwindcss-classes",
	...
	test.concurrent(
		"specify tailwind.config.js path (relative path)",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve("__tests__", "fixtures", "tailwind", "index.blade.php"),
				"--sort-tailwindcss-classes",
	...
	test.concurrent("custom html attributes order option", async () => {
		const cmdResult = await cmd.execute(binPath, [
			path.resolve(
				"__tests__",
				"fixtures",
				"custom_html_attributes_order.blade.php",
			),
			"--sort-html-attributes",
			"custom",
	...
	test.concurrent(
		"custom html attributes order option (with space)",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve(
					"__tests__",
	...
	test.concurrent(
		"custom html attributes order option (runtimeConfig)",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve(
					"__tests__",
	...
	test.concurrent("--end-of-line option", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve("__tests__", "fixtures", "formatted.end_of_line.blade.php"),
		);
	...
	test.concurrent("endOfLine option (runtime config)", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve(
				...
				"runtimeConfig",
				"endOfLine",
				"formatted.end_of_line.blade.php",
			),
		);
	...
	test.concurrent("cli argument test (no single quote)", async () => {
		...
		const formatted = fs.readFileSync(
			path.resolve(
				"__tests__",
				"fixtures",
				"formatted.single_quote_index.blade.php",
			),
		);
	...
	test.concurrent(
		"runtime config test (wrap attributes min attrs)",
		async () => {
			const cmdResult = await cmd.execute(binPath, [
				path.resolve(
					"__tests__",
