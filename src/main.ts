...
class BladeFormatter {
	...
	async readRuntimeConfig(
		...
		try {
			...
			process.stdout.write(
				chalk.red.bold(
					`\nBlade Formatter Config Error: ${nodepath.basename(
						configFile,
					)}\n\n`,
				),
			);
			process.stdout.write(
				`\`${error.errors[0].instancePath.replace("/", "")}\` ${
	...
	async processPaths() {
		await Promise.all(
			_.map(this.paths, async (path: any) => this.processPath(path)),
		);
	}
	...
	async processPath(path: any) {
		await BladeFormatter.globFiles(path)
			.then((paths: any) =>
				_.map(paths, (target: any) => nodepath.relative(".", target)),
			)
	...
	printFormattedFiles() {
		...
		if (!this.options.write) {
			if (this.options.checkFormatted) {
				process.stdout.write(
					`\nAbove file(s) are formattable. Forgot to run formatter? Use ${chalk.bold(
						"--write",
					)} option to overwrite.\n`,
				);
			}
		...
		_.each(this.formattedFiles, (path: any) =>
