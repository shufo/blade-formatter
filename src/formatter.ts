...
export default class Formatter {
	...
	formatAsHtml(data: any) {
		const options = {
			...
			wrap_attributes_min_attrs: util.optional(this.options)
			...
			max_preserve_newlines: util.optional(this.options).noMultipleEmptyLines
				? 1
				: undefined,
		...
		return _.replace(
			...
			(_match, p1, p2) => {
				...
				if (this.options.tailwindcssConfigPath) {
					const options = {
						tailwindConfigPath: this.options.tailwindcssConfigPath,
					};
				...
				if (this.options.tailwindcssConfig) {
					const options: any = {
						tailwindConfig: this.options.tailwindcssConfig,
					};
					return sortClasses(p2, options);
				}
	...
	async sortHtmlAttributes(content: string) {
		const strategy: SortHtmlAttributes =
			this.options.sortHtmlAttributes ?? "none";
	...
	async preserveComponentAttribute(content: string) {
		const prefixes =
			Array.isArray(this.options.componentPrefix) &&
			this.options.componentPrefix.length > 0
	...
	storeHtmlTag(value: string) {
		return this.getHtmlTagPlaceholder(
	...
	restorePhpBlock(content: any) {
		return this.restoreRawPhpBlock(content).then((target) =>
			this.restoreRawPropsBlock(target),
		);
	}
		...
		return replaceAsync(
			...
			async (match: any, p1: number) => {
				...
				if (isOnSingleLine && isMultipleStatements) {
					...
					rawBlock = (
						await util.formatStringAsPhp(`<?php\n${rawBlock}\n?>`, this.options)
					).trim();
					...
					rawBlock = (
						await util.formatStringAsPhp(`<?php${rawBlock}?>`, this.options)
					).trimEnd();
				} else {
			...
			async (match: string) => {
				...
				formatted = await replaceAsync(
					...
					async (match: any, p1: number) => {
						...
						if (
							this.isInline(rawBlock) &&
							(await this.isMultilineStatement(rawBlock))
						) {
							rawBlock = (
			...
			replaceAsync(
				...
				async (_match: any, p1: any) => {
					...
					try {
						...
						const result = (
							await util.formatStringAsPhp(this.rawPhpTags[p1], this.options)
						)
							.trim()
							.trimEnd();
				...
				(_match: any, p1: number) => {
					...
					const options = {
						...
						wrap_attributes_min_attrs: util.optional(this.options)
							.wrapAttributesMinAttrs,
						indent_inner_html:
							util.optional(this.options).indentInnerHtml || false,
						extra_liners: util.optional(this.options).extraLiners,
					...
					async (match2: string, p2: string, p3: string) => {
						try {
							const formatted = (
								...
								.trim()
								.substring(4);
							return `${p2}${this.indentComponentAttribute(
								indent.indent,
								formatted,
							)}`;
						} catch (error) {
					...
					async (match2: string, p3: string, p4: string) => {
						try {
							const formatted = (
								...
								.trim()
								.substring(4);
							return `${p3}${this.indentComponentAttribute(
								indent.indent,
								formatted,
							)}`;
						} catch (error) {
	...
	restoreXData(content: any) {
		return _.replace(
			...
			(_match: any, p1: any) => {
				...
				const indentLevel =
					indent.amount / (this.indentCharacter === "\t" ? 4 : 1);
	...
	async formatAsBlade(content: any) {
		...
		const formatted = registry
			...
			.then((tokenizedLines: any) =>
				this.formatTokenizedLines(splittedLines, tokenizedLines),
			)
			.catch((err: any) => {
				throw err;
			});
